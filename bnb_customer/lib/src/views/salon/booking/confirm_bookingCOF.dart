...
class confirmCOF<T> extends ConsumerStatefulWidget {
  static const route = "/confirmCOF";
  final bool
      isLocal; // This is true when we're booking straight without using the payroc implementation
  ...
  const confirmCOF({
    ...
    this.isLocal =
        false, // This is true when we're booking straight without using the payroc implementation
  }) : super(key: key);
  ...
  @override
  ConsumerState<confirmCOF> createState() => _confirmCOFState();
}

class _confirmCOFState extends ConsumerState<confirmCOF> {
  @override
  void initState() {
    if (widget.isLocal == false) {
      ...
      print('@@@@@@@@');
      TransactionApi()
          .getAllAppointmentWithTransactionCardOnFile(widget.transactionID)
          .listen((event) {
        if (event.isNotEmpty) {
          appointment = event[0];
          print('@@@@@@@@');
