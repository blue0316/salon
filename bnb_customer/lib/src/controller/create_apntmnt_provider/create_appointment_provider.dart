...
// gets list of master providing sevice
  ...
  getSlotsForSalonOwnerTye({required DateTime day, required BuildContext context, required bool showNotWorkingToast, required List<ServiceAndMaster> masterAndservice}) {
    ...
    serviceAgainstMaster.forEach((element) {
      element.NotCommonvalidSlots = element.validSlots!.where((elem) => !validSlots.contains(elem)).toList();
    });
    notifyListeners();
  }
  ...
  Future<bool> finishBooking({
    ...
    if (ownerType == OwnerType.singleMaster) {
      ...
      for (AppointmentModel app in appointmentModelSalonOwner!) {
        ...
        await blockTimeSalonOwnerMaster(app, serviceAgainstMaster.where((element) => element.service?.serviceId == app.services.first.serviceId).toList().isNotEmpty ? serviceAgainstMaster.where((element) => element.service?.serviceId == app.services.first.serviceId).toList()[0].master! : getMasterProvidingService(chosenServices.where((element) => element.serviceId == app.services.first.serviceId).toList()[0])[0]);
