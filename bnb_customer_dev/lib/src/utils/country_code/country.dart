...
import 'src/country_parser.dart';
...
class Country {
  ...
  bool startsWith(String query, CountryLocalizations? localizations) {
    ...
    return phoneCode.startsWith(_query.toLowerCase()) || name.toLowerCase().startsWith(_query.toLowerCase()) || countryCode.toLowerCase().startsWith(_query.toLowerCase()) || (localizations?.countryName(countryCode: countryCode)?.toLowerCase().startsWith(_query.toLowerCase()) ?? false);
  }
  ...
  String get flagEmoji => CountryCodeUtils.countryCodeToEmoji(countryCode);
}
