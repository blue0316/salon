...
typedef SingleMarkerBuilder<T> = Widget? Function(BuildContext context, DateTime day, T event);

/// Signature for a function that creates an event marker for a given `day`.
/// Contains a list of `events` associated with that `day`.
typedef MarkerBuilder<T> = Widget? Function(BuildContext context, DateTime day, List<T> events);
...
typedef HighlightBuilder = Widget? Function(BuildContext context, DateTime day, bool isWithinRange);
