...
class SalonProfileProvider with ChangeNotifier {
  ...
  bool isAppbarMenuHovered = false;

  changeHoveredAppBarMenu(bool value) {
    isAppbarMenuHovered = value;
    notifyListeners();
  }

  int appbarHoveredIndex = -1;

  void onEnterAppbar(int index) {
    appbarHoveredIndex = index;
    notifyListeners();
  }

  void onExitAppbar(int index) {
    appbarHoveredIndex = -1;
    notifyListeners();
  }

  int serviceHoveredIndex = -1;

  void onEnterService(int index) {
    serviceHoveredIndex = index;
    notifyListeners();
  }

  void onExitService(int index) {
    serviceHoveredIndex = -1;
    notifyListeners();
  }
  ...
  Country selectedCountry = countryCodes.map((country) => Country.from(json: country)).toList().first;
