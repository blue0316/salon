...
class _PayDialogState extends ConsumerState<PayDialog> {
  ...
  void initState() {
    super.initState();
    // TERMINALID:MerchatRef:DATETIME:action:SECRET
// bnbUkraine20211!
    var bytesToHash = utf8.encode(
    ...
    _iframeElement2.src = 'https://testpayments.worldnettps.com/merchant/securecardpage?TERMINALID=${widget.terminalId ?? "5363001"}&DATETIME=${formatter.format(timeNow)}&HASH=$hash&CURRENCY=${widget.currency ?? "USD"}&MERCHANTREF=${widget.transactionId ?? "Glamiris181234${timeNow.day}${timeNow.hour}${timeNow.minute}${timeNow.second}"}&ACTION=register&RECEIPTPAGEURL=https://glmrs.space/payroc-loader'; // https://api.glmrs.space/api/v1/payroc/receipt';
