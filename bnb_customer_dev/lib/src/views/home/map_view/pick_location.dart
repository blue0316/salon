...
class _PickLocationState extends ConsumerState<PickLocation> {
  ...
  Future _createMarker() async {
    ...
    if (_locationMarker == null) {
      final Uint8List mapSelectedSalonMarker = await getBytesFromAsset(AppIcons.mapSelectedSalonMarker, 90);
  ...
  setMarkers() async {
    ...
    setState(() {
      _marker.add(Marker(markerId: MarkerId('${DateTime.now().millisecondsSinceEpoch}'), position: _salonSearchProvider.tempCenter, icon: _locationMarker!));
    });
  }
        ...
        children: [
          ...
          Container(
            ...
            child: BnbMaterialButton(
              minWidth: DeviceConstraints.getResponsiveSize(context, width * 0.8, width * 0.6, width * 0.4),
